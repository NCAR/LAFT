...
| `data/exp2_jd/` | The Fortran reference. `src/` has the scheme (`kessler.F90`, `kessler_update.F90`), the CCPP metadata, the reference driver (`JD_kessler_driver.F90`), the input generator, a Makefile, the scalability benchmark source, and the golden compiler logs. `fortran_io/` has the 128-column × 56-level inputs and the Fortran outputs every translation is compared against. |
