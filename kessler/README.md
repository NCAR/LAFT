...
| `_compare_results/` | Cross-model analysis of the April 2026 runs: the performance comparison report, accuracy and GPU scalability plots, and Nsight Systems profiling reports. Also the profiling runner behind the Nsight Systems reports (`LLMx_jd_profiling_run.py`, run by `../LAFT/pbsJobs/compare_profiling_LLMs.sh`) and the two scalability scripts that produced the Fortran-vs-JAX figure (`scalability_benchmark_LLMs.py`, `plot_Fortran_vs_LLMs_scalability.py`) and, under `outputs/scalability/`, the timing JSONs they read and wrote; the serial Fortran half is `data/exp2_jd/src/scalability_benchmark_fortran.F90`, run by `../LAFT/pbsJobs/compare_scalability_fortran.sh`. |
