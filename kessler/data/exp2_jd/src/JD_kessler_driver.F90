program test_kessler_driver
  ...
  do i = 1, ncol
     do k = 1, nz
        ...
        zm(i,k)    = zm(i,k)
        exner(i,k) = arr(i) * 0.97_kind_phys
     end do
  ...
  !------------------------------------------------------
  ! Write inputs to files (for JAX driver)
  ! Run from project root: ../fortran_io/inputs/ must exist
  !------------------------------------------------------
  open(unit=20, file='../fortran_io/inputs/cpair.txt',  status='replace', action='write')
  open(unit=21, file='../fortran_io/inputs/rair.txt',   status='replace', action='write')
  open(unit=22, file='../fortran_io/inputs/rho.txt',    status='replace', action='write')
  open(unit=23, file='../fortran_io/inputs/z.txt',      status='replace', action='write')
  open(unit=24, file='../fortran_io/inputs/pk.txt',     status='replace', action='write')
  open(unit=25, file='../fortran_io/inputs/theta.txt',  status='replace', action='write')
  open(unit=26, file='../fortran_io/inputs/qv.txt',     status='replace', action='write')
  open(unit=27, file='../fortran_io/inputs/qc.txt',     status='replace', action='write')
  open(unit=28, file='../fortran_io/inputs/qr.txt',     status='replace', action='write')
  open(unit=29, file='../fortran_io/inputs/precl.txt',  status='replace', action='write')
  open(unit=30, file='../fortran_io/inputs/meta.txt',   status='replace', action='write')

  do i = 1, ncol
     write(20, '(*(ES26.20,1X))') cpair(i,1:nz)
     write(21, '(*(ES26.20,1X))') rair(i,1:nz)
     write(22, '(*(ES26.20,1X))') rho(i,1:nz)
     write(23, '(*(ES26.20,1X))') z(i,1:nz)
     write(24, '(*(ES26.20,1X))') pk(i,1:nz)
     write(25, '(*(ES26.20,1X))') theta(i,1:nz)
     write(26, '(*(ES26.20,1X))') qv(i,1:nz)
     write(27, '(*(ES26.20,1X))') qc(i,1:nz)
     write(28, '(*(ES26.20,1X))') qr(i,1:nz)
  end do
  write(29, '(*(ES26.20,1X))') precl(1:ncol)

  write(30, '(A,I0)')  'ncol=',     ncol
  write(30, '(A,I0)')  'nz=',       nz
  write(30, '(A,F4.1)') 'dt=',      dt
  write(30, '(A,I0)')  'lyr_surf=', lyr_surf
  write(30, '(A,I0)')  'lyr_toa=',  lyr_toa
  write(30, '(A,I0)')  'seed=',     0

  close(20); close(21); close(22); close(23); close(24)
  close(25); close(26); close(27); close(28); close(29); close(30)

  print *, 'Inputs written to ../fortran_io/inputs/'
  ...
  !------------------------------------------------------
  ! Write outputs to ../fortran_io/outputs/
  ! Same files and format as kessler_fortran_driver.F90
  !------------------------------------------------------
  call write_2d('../fortran_io/outputs/theta.txt',  theta,  ncol, nz)
  call write_2d('../fortran_io/outputs/qv.txt',     qv,     ncol, nz)
  call write_2d('../fortran_io/outputs/qc.txt',     qc,     ncol, nz)
  call write_2d('../fortran_io/outputs/qr.txt',     qr,     ncol, nz)
  call write_1d('../fortran_io/outputs/precl.txt',  precl,  ncol)
  call write_2d('../fortran_io/outputs/relhum.txt', relhum, ncol, nz)

  print *, 'Outputs written to ../fortran_io/outputs/'

contains

  subroutine write_2d(filename, arr, n1, n2)
     character(len=*), intent(in) :: filename
     real(kind_phys),  intent(in) :: arr(n1, n2)
     integer,          intent(in) :: n1, n2
     integer :: u, i, j
     open(newunit=u, file=filename, status='replace', action='write')
     do i = 1, n1
        write(u, '(*(ES30.20E3,1X))') (arr(i,j), j=1,n2)
     end do
     close(u)
  end subroutine write_2d

  subroutine write_1d(filename, arr, n)
     character(len=*), intent(in) :: filename
     real(kind_phys),  intent(in) :: arr(n)
     integer,          intent(in) :: n
     integer :: u
     open(newunit=u, file=filename, status='replace', action='write')
     write(u, '(*(ES30.20E3,1X))') arr
     close(u)
